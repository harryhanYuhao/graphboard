 ...
 * @returns {string}
 */
export function compute_api_version() {
    let deferred1_0;
    let deferred1_1;
    try {
        const ret = wasm.compute_api_version();
        deferred1_0 = ret[0];
        deferred1_1 = ret[1];
 ...
 * @returns {any}
 */
export function compute_tensor(input, on_progress) {
    const ret = wasm.compute_tensor(input, isLikeNone(on_progress) ? 0 : addToExternrefTable0(on_progress));
    if (ret[2]) {
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('zxw_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
