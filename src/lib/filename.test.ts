// `toSafeFilename` turns a title into a filesystem-safe basename. The
// sanitization rules are an implicit contract for JSON export — pin them
// so the sanitizer can't quietly start emitting broken filenames.
...
describe("toSafeFilename", () => {
  ...
  it("truncates after sanitizing, so a replaced dash still counts", () => {
    // 79 legal chars + one illegal char: sanitize first (80 chars), then slice keeps all 80.
  ...
  it("returns the empty string for an empty input (pinned)", () => {
    // No fallback; the caller guards with `state.title || "graph-board"`.
